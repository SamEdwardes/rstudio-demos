  ...
  fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        p(
          "If you encounter any issues with this application, please submit bugs to ",
          a("GitHub", href = "https://github.com/colearendt/shiny-session-info")
        ),
        p("Use the listviewers to the right for exploring Shiny session state"),
        br(),
        h4("Important Notes"),
        p(
          "This app has shown fragility with a large number of groups. If you see errors and have a large number of groups, please refresh"
        )
      ),
...
server <- function(input, output, session) {
  ...
  output$sessionInfo <- renderJsonedit({
    tryCatch(
      {
        ...
        calt_clean_2 <- calt_clean[which(
          !calt_class %in% c("reactivevalues", "shinyoutput")
        )]
    ...
    jsonedit(calt_final, mode = 'view', modes = list('view'))
  })
