...
class Penguin(BaseModel):
    ...
    island: str | None = Field(default=None)
    bill_length_mm: float | None = Field(default=None)
    bill_depth_mm: float | None = Field(default=None)
    flipper_length_mm: float | None = Field(default=None)
    body_mass_g: float | None = Field(default=None)
...
def penguins(sample_size: int | None = None):
    ...
    penguins_df = pl.read_csv(url, null_values=["NA"])
    if sample_size:
...
def raw_penguins(sample_size: int | None = None):
    ...
    url = "https://raw.githubusercontent.com/allisonhorst/palmerpenguins/main/inst/extdata/penguins.csv"
    penguins_raw_df = pl.read_csv(url, null_values=["NA"])
    if sample_size:
        penguins_raw_df = penguins_raw_df.sample(sample_size, null_values=["NA"])
